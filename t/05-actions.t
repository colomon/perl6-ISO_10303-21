...
isa-ok $match, Match, "<parameter> matches #12";
ok $match, "<parameter> matches #12";
isa-ok $match.ast, Str, "and ast is Str";
is $match.ast, "#12", "and ast is #12";

$match = ISO_10303_21::Grammar.parse("#12",
                                     :rule<untyped_parameter>,
                                     :actions(ISO_10303_21::Actions.new));
isa-ok $match, Match, "<untyped_parameter> matches #12";
...
$match = ISO_10303_21::Grammar.parse("(#12, #42)",
                                     ...
                                     :actions(ISO_10303_21::Actions.new));
isa-ok $match, Match, "<list_of_parameters> matches (#12, #42)";
ok $match, "<list_of_parameters> matches (#12, #42)";
isa-ok $match.ast, Array, "and the ast is Array";
is +$match.ast, 2, "and the ast has length 2";
isa-ok $match.ast[0], Str, "and ast[0] is Str";
isa-ok $match.ast[1], Str, "and ast[1] is Str";
...
$match = ISO_10303_21::Grammar.parse("(#12, #42)",
                                     :rule<parameter>,
                                     :actions(ISO_10303_21::Actions.new));
isa-ok $match, Match, "<parameter> matches (#12, #42)";
...
$match = ISO_10303_21::Grammar.parse("#12, #42",
                                     :rule<parameter_list>,
                                     :actions(ISO_10303_21::Actions.new));
isa-ok $match, Match, "<parameter_list> matches #12, #42";
...
$match = ISO_10303_21::Grammar.parse("LEAF('Blah', #42)",
                                     :rule<simple_record>,
                                     :actions(ISO_10303_21::Actions.new));
isa-ok $match, Match, "<simple_record> matches LEAF('Blah', #42)";
...
isa-ok $match.ast.parameters[1], Str, "and parameters[1] is Str";
...
$match = ISO_10303_21::Grammar.parse("LEAF()",
                                     :rule<simple_record>,
                                     :actions(ISO_10303_21::Actions.new));
isa-ok $match, Match, "<simple_record> matches LEAF()";
...
$match = ISO_10303_21::Grammar.parse("(LEAF('Blah', #42) BRANCH(#1949))",
                                     ...
                                     :actions(ISO_10303_21::Actions.new));
isa-ok $match, Match, "<subsuper_record> matches LEAF('Blah', #42) BRANCH(#1949)";
ok $match, "<subsuper_record> matches LEAF('Blah', #42) BRANCH(#1949)";
is +$match.ast, 2, "Got two records";
isa-ok $match.ast[0], ISO_10303_21::Record, "and the ast[0] is ISO_10303_21::Record";
isa-ok $match.ast[1], ISO_10303_21::Record, "and the ast[1] is ISO_10303_21::Record";
...
$match = ISO_10303_21::Grammar.parse("PRODUCT_RELATED_PRODUCT_CATEGORY('detail',\$,(#8))",
                                     :rule<simple_record>,
                                     :actions(ISO_10303_21::Actions.new));
isa-ok $match, Match, "<simple_record> matches PRODUCT_RELATED_PRODUCT_CATEGORY('detail',\$,(#8))";
...

{
    ...
    $match = ISO_10303_21::Grammar.parse($step,
                                         :rule<data_section>,
                                         :actions($file));
    isa-ok $match, Match, "<data_section> matches STEP file snippet";
...

{
    ...
    my $match = ISO_10303_21::Grammar.parse($file-data, 
                                            :rule<exchange_file>,
                                            :actions($file));
    isa-ok $match, Match, "Grammar.<exchange_file> returns a Match for file...";
